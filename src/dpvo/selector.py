...
class PatchSelector:
    ...
    def _grid(self, scores):
        ...
        size = (h2, w2)
        scores_grid = F.unfold(
            scores_grid, kernel_size=size, stride=size
        )  # (B*N,1*h2*w2,GRID*GRID)
        # scores_grid = F.softmax(scores_grid, dim=-2)
        scores_grid = scores_grid.view(
            b, n, h2, w2, self.GRID**2
        )  # (b,n,h2,w2,GRID*GRID)
        return scores_grid
    ...
    def _multi(self, scores, patches_per_image):
        ...
        b, n, h, w = scores.shape
        # 1) avg pooling
        avg_scores = F.avg_pool2d(
            scores, kernel_size=self.KERNEL_SIZE, stride=self.KERNEL_SIZE
        )
        ...
        if self.grid:
            ...
            scores_t = (
                scores_grid.view(b * n, h2 * w2, (self.GRID**2))
                .transpose(-2, -1)
                .contiguous()
                .view(b * n * (self.GRID**2), h2 * w2)
            )  # (b,n,h2*w2,GRID*GRID) -> (b,n,GRID*GRID,h2*w2) -> (b*n*GRID*GRID,h2*w2)
            scores_t += 1e-7  # to fulfill non-zero sum
            idx = torch.multinomial(
                scores_t, patches_per_image // (self.GRID**2)
            )  # (b*n*GRID*GRID,patches_per_image/(GRID*GRID))
            idx = idx.view(b * n, self.GRID**2, -1).transpose(
                -2, -1
            )  # -> (b*n,patches_per_image/(GRID*GRID),GRID*GRID)
            idx = self._grid2_idx_up(idx, scores_grid)  # (b*n,patches_per_image)
        else:
            avg_scores = avg_scores.view(b * n, -1)  # (b*n,h1*w1)
            idx = torch.multinomial(
                avg_scores, patches_per_image
            )  # (b*n,patches_per_image)

        # 3) multinomial sampling for index
        idx_gather = idx[..., None].repeat(
            1, 1, self.KERNEL_SIZE * self.KERNEL_SIZE
        )  # -> (b*n,patches_per_image,KERNEL_SIZE*KERNEL_SIZE)
        avg_windows = F.unfold(
            scores.view(b * n, 1, h, w),
            kernel_size=self.KERNEL_SIZE,
            stride=self.KERNEL_SIZE,
            padding=1,
        ).transpose(-2, -1)  # -> (b*n,#windows,1*KERNEL_SIZE*KERNEL_SIZE)
        avg_windows_multi = torch.gather(
            avg_windows, -2, idx_gather
        )  # (b*n,patches_per_image,KERNEL_SIZE*KERNEL_SIZE)
        avg_windows_multi += 1e-7  # to fulfill non-zero sum
        idx_offset = torch.multinomial(avg_windows_multi.flatten(0, 1), 1).view(
            b * n, patches_per_image
        )  # (b*n*patches_per_image,1) -> (b*n,patches_per_image)
        x_offset = idx_offset % self.KERNEL_SIZE  # (b*n,patches_per_image)
        y_offset = torch.div(
            idx_offset, self.KERNEL_SIZE, rounding_mode="floor"
        )  # (b*n,patches_per_image)
    ...
    def _topk(self, scores, patches_per_image):
        ...
        max_windows = F.unfold(
            scores.view(b * n, 1, h, w),
            kernel_size=self.KERNEL_SIZE,
            stride=self.KERNEL_SIZE,
        )  # (b*n,1*KERNEL_SIZE*KERNEL_SIZE,#windows)
        max_scores, max_idx = torch.max(max_windows, dim=-2)  # (b*n,#windows)
        max_scores = max_scores.view(
            b, n, h // self.KERNEL_SIZE, w // self.KERNEL_SIZE
        )  # (b,n,h1,w1)
        ...
        if self.grid:
            ...
            _, idx = torch.topk(
                max_scores.view(b * n, h1 * w1), patches_per_image, dim=-1
            )  # (b*n,patches_per_image)
    ...
    def _nms(self, scores, patches_per_image):
        ...
        b, n, h, w = scores.shape
        # 1) max pooling (with indices)
        max_scores, max_idx = F.max_pool2d(
            scores,
            kernel_size=self.KERNEL_SIZE,
            stride=self.KERNEL_SIZE,
            return_indices=True,
        ...
        boxes_x2y2 = (
            boxes_x1y1
            + torch.as_tensor(
                [2 * self.NMS_RADIUS, 2 * self.NMS_RADIUS], device="cuda"
            )[None, None]
        )
        ...
        if self.grid:
            ...
            idx_boxes = (
                torch.arange(0, b * n * 4, 4, device="cuda")
                .view(-1, 1)
                .repeat(1, h1 * w1)
                .view(-1)
            )  # (b*n*h1*w1)
    ...
    def __call__(self, scores, patches_per_image):
        ...
        padding_h_top, padding_h_bottom = (
            ...
            if (padding_h % 2) == 0
            else (padding_h // 2, padding_h // 2 + 1)
        )
        padding_w_left, padding_w_right = (
            (padding_w // 2, padding_w // 2)
            if (padding_w % 2) == 0
            else (padding_w // 2, padding_w // 2 + 1)
        )
