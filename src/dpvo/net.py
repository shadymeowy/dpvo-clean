...
from .selector import PatchSelector, Scorer, SelectionMethod
...
class ePatchifier(nn.Module):
    def __init__(
        ...
        dim_fnet=128,
        dim=32,
        patch_selector=SelectionMethod.SCORER,
    ):
        super(ePatchifier, self).__init__()
        self.patch_size = patch_size
        self.dim_inet = (
            dim_inet  # dim of context extractor and hidden state (update operator)
        )
        self.dim_fnet = dim_fnet  # dim of matching extractor
        self.patch_selector = patch_selector.lower()
        self.fnet = BasicEncoder4(
            output_dim=self.dim_fnet, dim=dim, norm_fn="instance", bins=5
        )  # matching-feature extractor
    ...
    def forward(
        ...
        if self.patch_selector == SelectionMethod.GRADIENT:
            g = self.__image_gradient(images)  # gradient map (b,n_frames,h/4-1,w/4-1)

            if self.training:
                patch_selector_fn = PatchSelector("3xrandom")
            else:
                patch_selector_fn = PatchSelector(
                    scorer_eval_mode, grid=scorer_eval_use_grid
                )
            ...
            if self.training:
                x = torch.randint(
                    0, w - 2, size=[n, 3 * patches_per_image], device="cuda"
                )
                y = torch.randint(
                    0, h - 2, size=[n, 3 * patches_per_image], device="cuda"
                )

                coords = torch.stack(
                    [x, y], dim=-1
                ).float()  # (n_frames,3*patches_per_image,2)
                scores = altcorr.patchify(scores[0, :, None], coords, 0).view(
                    n, 3 * patches_per_image
                )  # extract patches of scorer map

                vx, ix = torch.sort(
                    scores, dim=1
                )  # sort by score (n_frames,3*patches_per_image)
                x = x + 1
                y = y + 1
                x = torch.gather(
                    x, 1, ix[:, -patches_per_image:]
                )  # choose patch idx with largest score
                y = torch.gather(y, 1, ix[:, -patches_per_image:])
                scores = (
                    vx[:, -patches_per_image:].contiguous().view(n, patches_per_image)
                )
            else:
                patch_selector_fn = PatchSelector(
                    scorer_eval_mode, grid=scorer_eval_use_grid
                ...
                coords = torch.stack(
                    [x, y], dim=-1
                ).float()  # (b*n,patches_per_image,2)
                scores = altcorr.patchify(scores[0, :, None], coords, 0).view(
                    n, patches_per_image
                )  # extract weights of scorer map
        ...
        imap = altcorr.patchify(imap[0], coords, 0).view(
            b, -1, self.dim_inet, 1, 1
        )  # [B, n_images*n_patches_per_image, dim_inet, 1, 1]
        gmap = altcorr.patchify(fmap[0], coords, P // 2).view(
            b, -1, self.dim_fnet, P, P
        )  # [B, n_images*n_patches_per_image, dim_fnet, 3, 3]

        if return_color:
            clr = (
                altcorr.patchify(
                    images[0].abs().sum(dim=1, keepdim=True), 4 * (coords + 0.5), 0
                )
                .clamp(min=0, max=255)
                .view(b, -1, 1)
            )
...
class Patchifier(nn.Module):
    ...
    def forward(
        ...
        if centroid_sel_strat == "GRADIENT_BIAS":
            ...
            g = altcorr.patchify(g[0, :, None], coords, 0).view(
                n, 3 * patches_per_image
            )
...
class eVONet(nn.Module):
    ...
    def forward(
        ...
        if self.patch_selector == SelectionMethod.SCORER:
            fmap, gmap, imap, patches, ix, scores = self.patchify(
                images, patches_per_image=patches_per_image, disps=disps
            )
        else:
        ...
        net = torch.zeros(
            b, len(kk), self.dim_inet, device="cuda", dtype=torch.float
        )  # init hidden state
        ...
        while len(traj) < STEPS:
            ...
            coords = pops.transform(Gs, patches, intrinsics, ii, jj, kk)
            coords1 = coords.permute(
                0, 1, 4, 2, 3
            ).contiguous()  # (B,edges,P,P,2) -> (B,edges,2,P,P)
            ...
            if self.patch_selector == SelectionMethod.SCORER:
                coords_full = pops.transform(
                    Gs, patches, intrinsics, ii, jj, kk
                )  # p_ij (B,close_edges,P,P,2)
                coords_gt_full, valid_full = pops.transform(
                    Ps, patches_gt, intrinsics, ii, jj, kk, valid=True
                )
                coords = pops.transform(
                    Gs, patches, intrinsics, ii[k], jj[k], kk[k]
                )  # p_ij (B,close_edges,P,P,2)
                ...
                traj.append(
                    (
                        ...
                        scores,
                        valid_full[0, k],
                        coords_full.detach()[0, k],
                        coords_gt_full.detach()[0, k],
                        weight.detach()[0, k],
                        kk[k],
                        dij[k],
                    )
