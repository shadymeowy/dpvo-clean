...
class DEIO:
    def __init__(
        ...
        self.args = cfg  # The parameter file is passed in
        self.load_weights(network)  # Read network weights
        self.is_initialized = False  # Is visual initialized
        self.enable_timing = False  # TODO timing in param

        self.M = self.cfg.PATCHES_PER_FRAME  # (default: 96) number of patches
        self.N = (
            self.cfg.BUFFER_SIZE
        )  # max number of keyframes (default: 2048), number of buffers
        ...
        ### state attributes ###
        self.tlist = []  # timestamp list (should be on the cpu)
        ...
        self.image_ = torch.zeros(
            self.ht, self.wd, 3, dtype=torch.uint8, device="cpu"
        ...
        # Define an instance of the PatchGraph class
        ...
        self.state = MultiSensorState()  # Pose estimation state
    ...
    @property
    def poses(self):  # Use poses for calling, use self.pg.poses_ for writing updates
    ...
    def terminate(self):
        ...
        if self.cfg.LOOP_CLOSURE:  # 0: #1:
            ...
            tstamps = np.array(self.poses_save, dtype=np.float64)[
                :, 0
            ]  # Get timestamps
    ...
    def append_factors(self, ii, jj):
        ...
        self.pg.kk = torch.cat(
            [self.pg.kk, ii]
        )  # The inserted ii is actually the patch index, kk
        self.pg.ii = torch.cat(
            [self.pg.ii, self.ix[ii]]
            ...
            for i in range(k, self.n - 1):
                ...
                if i == k:
                    for iii in range(len(self.state.preintegrations_meas[i])):
                        dd = self.state.preintegrations_meas[
                            i
                        ][
                            iii
    ...
    def __run_DBA(self, target, weight, lmbda, ii, jj, kk, t0, t1, eff_impl):
        ...
        if self.last_t1 != t1 or self.last_t0 != t0:
            if self.last_t0 >= t0:
                if not eff_impl:  # If not doing GBA
                    t0 = self.last_t0
            else:  # self.last_t0 < t0, which means the previous t0 is smaller than the current t0
                ...
                if len(marg_ii) > 0:
                    ...
                    bafactor.hessian(H, v)  # Marginalization
                    ...
                    vis_factor = CustomHessianFactor(
                        marg_result, Hg, vg
                    )  # Build visual factors

                    # graph.push_back(vis_factor) #  Add visual factors to the marginalization graph
                ...
                for i in range(self.last_t0, marg_t1):
                    if i < t0:
                        if X(i) not in marg_paras:
                            marg_paras.append(X(i))
                        if not self.ignore_imu:  # If IMU is not ignored
                ...
                if self.reinit:  # If re-initializing
            ...
            if not eff_impl:  # If there is GBA, do not update
                self.last_t0 = t0
            self.last_t1 = t1

        """ optimization multi-sensor joint backend optimization """
        self.cur_graph = gtsam.NonlinearFactorGraph()  # The current graph maintained during gtsam optimization (the naming is the same as for marginalization, but the meaning is different~)
        ...
        if not self.ignore_imu:
            for i in range(t0, t1):
                if i > t0:
                    ...
                    self.cur_graph.add(imu_factor)  # Add IMU pre-integration
        ...
        for i in sorted(keys):
            if i >= t0 and i < t1:
                for iii in range(len(self.prior_factor_map[i])):
                    self.cur_graph.push_back(
                        self.prior_factor_map[i][iii]
                    )  # Add prior factors

        # marginalization factor
        # if self.marg_factor is not None:
        if (
            not eff_impl and self.marg_factor is not None
        ...
        dx = torch.zeros(
            [(t1 - t0) * 6], dtype=torch.float64, device="cpu"
        ...
        for iter in range(2):
            if iter > 0:
                self.cur_graph.resize(
                    self.cur_graph.size() - 1
            ...
            for i in range(t0, t1):  # Give initial values for the states
                initial.insert(
                    X(i), self.state.wTbs[i]
            ...
            if not self.ignore_imu:  # If IMU is not ignored, add the bias
                for i in range(t0, t1):
            ...
            self.cur_result = optimizer.optimize()  # Optimize using gtsam
            ...
            dx = torch.tensor(gtsam.GTSAM2BA(dx, self.Tbc))  # Pose change
            _ = bafactor.retract(
                dx
    ...
    def update(self):
        coords = self.reproject()  # Perform reprojection
        ...
        with Timer("BA", enabled=self.enable_timing):
            try:
                if self.imu_enabled:  # If using imu
                    t1 = self.n
                    eff_impl_flag = False
                    ...
                    else:  # Run local BA optimization
                        t0 = (
    ...
    def init_IMU(self):
        ...
        cur_t = float(
            self.tlist[self.pg.tstamps_[self.t0]]
        ...
        for i in range(self.t0, self.t1):
            self.tlist[self.pg.tstamps_[i]]  # Get the timestamp of the current frame
            if i == self.t0:  # If it's the first frame
                self.state.init_first_state(
                    cur_t, np.zeros(3), np.eye(3), np.zeros(3)
                ...
                while (
                    self.all_imu[self.cur_imu_ii][0] < cur_t
            ...
            q = torch.tensor(
                Rotation.from_matrix(TTT[:3, :3]).as_quat()
            )  # Convert the rotation matrix to a quaternion
            t = TTT[:3, 3]
            if not self.imu_enabled:  # If IMU is not used (if it is false, it is initialized to false, so it will be executed)
                self.pg.poses_[i] = torch.cat([t, q])  # Assign values
                # gwp_donothing=1
    ...
    def __initialize(self):
        ...
        self.is_initialized = True  # Flag for completion of initialization
    ...
    def VisualIMUAlignment(self, t0, t1, ignore_lever, disable_scale=False):
        ...
        for i in range(0, t1):
            ...
            # self.disps[i] /= s
            self.patches_gt_[i, :, 2] /= s  # Rewrite the depth of all patches
    ...
    def init_VI(self):
        ...
        if var_g < self.cfg.VI_INIT_VAR_G and norm < self.cfg.VI_INIT_NORM:
            ...
            if not self.visual_only:  # If there is an IMU
            ...
            else:  # The following can be ignored
                # Report error
    ...
    def VIO_update(self):
        ...
        cur_t = float(
            self.tlist[self.pg.tstamps_[self.t1 - 1]]
        ...
        self.state.append_img(cur_t)  # Update state after inserting IMU
        ...
        ## predict pose (<5 ms)
        if self.imu_enabled:  # If IMU is used
            Twc = (self.state.wTbs[-1] * self.Tbc).matrix()  # The latest Twb*Tbc
            TTT = torch.tensor(np.linalg.inv(Twc))  # Get its inverse, which is Tcw
            q = torch.tensor(
                Rotation.from_matrix(TTT[:3, :3]).as_quat()
            )  # Get the quaternion
            t = TTT[:3, 3]
            self.pg.poses_[self.t1 - 1] = torch.cat(
                [t, q]
            )  # Initialize the current pose

        self.update()  # Perform update operation

        # Save and output the pose result
        poses = SE3(self.pg.poses_)  # Get pose
        TTT = np.matmul(
        ...
        self.keyframe()  # Keyframe management
        ...
        if (
            ...
            if self.visual_only == 1:  # IMU is not used, this is a passed parameter
                self.visual_only_init = True
            else:
                self.init_VI()  # Visual initialization will only be performed when IMU is used
    ...
    def __call__(self, tstamp, image, intrinsics, scale=1.0):
        ...
        self.tlist.append(tstamp)  # Timestamp, global time timestamp
        self.pg.tstamps_[self.n] = (
            self.counter
        ...
        if self.n > 0 and not self.is_initialized:  # Visual is not initialized yet
            thres = (
                2.0 if scale == 1.0 else scale**2
            )  # TODO adapt thres for lite version
            if (
                self.motion_probe() < thres
        ...
        if self.cfg.LOOP_CLOSURE:  # If loop closure is enabled (this should be the loop closure implemented in DPVO)
        ...
        if (
            self.n == 8 and not self.is_initialized
        ):  # Not initialized yet and meets 8 frames
            self.__initialize()  # Perform visual and inertial initialization
        elif self.is_initialized:
            self.VIO_update()  # Perform VIO update, which also includes the above two functions

        if self.cfg.CLASSIC_LOOP_CLOSURE:  # If classic loop closure is enabled
            self.long_term_lc.attempt_loop_closure(
                self.n
