...
test_split = ["MH%03d" % i for i in range(8)] + ["ME%03d" % i for i in range(8)]
...
def evaluate(config, net, split="validation", trials=1, plot=False, save=False):
    ...
    results_dict = dict(
        [("Tartan/{}".format(k), np.median(v)) for (k, v) in results.items()]
    )

    # write output to file with timestamp
    with open(
        osp.join(
            "TartanAirResults", datetime.datetime.now().strftime("%m-%d-%I%p.txt")
        ),
        "w",
    ) as f:
...
if __name__ == "__main__":
    ...
    if args.id >= 0:
        ...
        traj_est, tstamps = run(
            scene_path, cfg, args.weights, viz=args.viz, show_img=args.show_img
        )
