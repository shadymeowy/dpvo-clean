...
if __name__ == "__main__":
    ...
    for scene in scenes:
        ...
        if scene.startswith("living"):
            groundtruth = (
                args.iclnuim_dir / "TrajectoryGT" / f"livingRoom{scene[-6]}.gt.freiburg"
            )
        elif scene.startswith("office"):
            groundtruth = (
                args.iclnuim_dir / "TrajectoryGT" / f"traj{scene[-6]}.gt.freiburg"
            )
        traj_ref = file_interface.read_tum_trajectory_file(groundtruth)

        scene_results = []
        for i in range(args.trials):
            traj_est, timestamps = run(
                cfg,
                args.network,
                imagedir,
                "calib/icl_nuim.txt",
            ...
            if args.plot:
                ...
                plot_trajectory(
                    traj_est,
                    traj_ref,
                    f"ICL_NUIM {scene_name} Trial #{i + 1} (ATE: {ate_score:.03f})",
                    f"trajectory_plots/ICL_NUIM_{scene_name}_Trial{i + 1:02d}.pdf",
                    align=True,
                    correct_scale=True,
                )
            ...
            if args.save_trajectory:
                ...
                file_interface.write_tum_trajectory_file(
                    f"saved_trajectories/ICL_NUIM_{scene_name}_Trial{i + 1:02d}.txt",
                    traj_est,
                )
