...
def run(cfg, network, kittidir, sequence, stride=1, viz=False, show_img=False):
    ...
    reader = Process(
        target=kitti_image_stream, args=(queue, kittidir, sequence, stride, 0)
    )
    reader.start()
...
if __name__ == "__main__":
    ...
    for scene in kitti_scenes:
        ...
        print(
            f"Evaluating KITTI {scene} with {poses_ref.num_poses // args.stride} poses"
        )
        ...
        for trial_num in range(args.trials):
            ...
            if args.plot:
                plot_trajectory(
                    traj_est,
                    traj_ref,
                    f"kitti sequence {scene} Trial #{trial_num + 1}",
